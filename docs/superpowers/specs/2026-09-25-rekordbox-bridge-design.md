...
| `rekordbox/compare.py` | `compare_libraries(expected, actual)` reports, per written track, marker and mark, whether Rekordbox's export kept, rounded, changed, dropped or duplicated it, and which extra marks appeared. Used by the probe. |
...
2. Consider library tracks named by `--add` or `--cues`, and new `--add` files, which must exist. Run the cached `analyze_track` only for a track without a Rekordbox grid; a library track whose file is missing gets no grid, with the reason recorded.
...
4. Cues: for a track with cue requests, `place_cues` with the track's existing marks, its requests, and the profile. `build_rekordbox_import` accepts a `profile` that overrides the cue limits; the probe uses it to test more memory cues than the default allows.
...
- **Reader:** a synthetic fixture, `tests/data/rekordbox-collection.xml`, modelled on the 7.2.18 export (full attribute set, multi-line attributes, hot cues 0–7, memory cue without colour, streaming location, sampler track). Locations containing spaces, `%`, `#`, `&`, apostrophes, accented characters, and a drive root decode correctly. Malformed documents and documents with a DOCTYPE raise `InputError`.
